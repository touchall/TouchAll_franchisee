...
public class TNHttpMultiPartTask extends AsyncTask<BaseHttpResource, Void, BaseHttpResource[]> implements HttpPopupManager.PopupDialog.OnResultListener {
    ...
    public TNHttpMultiPartTask(Context c, FragmentManager fm) {
        super();
        this.mContext = c;
        this.mFm = fm;
    }
        ...
        try {
            ...
            if (param.getType() == HttpInfo.GET) {
                ...
                if (getSubUrl.length() > 0) {
                    getSubUrl = URLEncoder.encode(getSubUrl, "UTF-8");
                    szUrl += getSubUrl;
                }

                //szUrl = URLEncoder.encode( szUrl, "UTF-8" );
            }
            else if (param.getType() == HttpInfo.GET_EX){
                String getSubUrl = requestGetDataEx(param);
                if (getSubUrl.length() > 0) {
                    //getSubUrl = URLEncoder.encode(getSubUrl, "UTF-8");
                    szUrl += getSubUrl;
                }
            ...
            conn = ssl.postHttps(szUrl, HttpInfo.TIME_OUT, HttpInfo.TIME_OUT);
            // SJH   >>>>>>>>>>>>>>>>>>>>
...
//                String certification = param.getCertification();
//                if (certification != null && certification.length() > 0)
//                    conn.addRequestProperty("Certification", certification);
...
//                    addFormField(mp.name, mp.value);
    ...
    private String requestGetDataEx(BaseHttpResource param) {
        String getParamData = "";
        ArrayList<String> get_datas = param.getGetUrlData();
        if (debug) SLog.LogI("get data size:" + get_datas.size());
        for (int i = 0; i < get_datas.size(); i++) {
            String sub_url = get_datas.get(i);
            getParamData += sub_url;
            if (i < get_datas.size() - 1)
                getParamData += "/";
        }

        if (debug) SLog.LogI("sub url:" + getParamData);
        return getParamData;
    }
...
//                    try {
//                        getParamData += URLEncoder.encode(mp.value, "UTF-8");
